...
class PluginActionsLoader {
    ...
    loadActions() {
        return Promise.resolve()
            ...
            .then(pluginPacks => this._extractActions(pluginPacks));
    }

    _loadPluginPackModules() {
        const pluginPacks = this._kumoContext.settings().pluginPacks;
        return _.compact(_.keys(pluginPacks).map(name =>
            this._requireModule(this._pluginPackDir(name))
        ));
    }

    _extractActions(pluginPacks) {
        return _.flattenDeep(pluginPacks.map(pack =>
            pack.plugins().map(plugin => this._extractPluginActions(plugin, pack.prefix()))
        ));
    }
    ...
    _pluginPackDir(name) {
        const kumoDir = this._kumoContext.kumoDir();
